...
const DayCard = ({ day, month, events }) => {
  ...
  return (
    <div className="relative flex flex-col bg-white group">
      <span className="mx-2 my-1 text-xs font-bold">{day} {day === 1 && month}</span>

      <div className="flex flex-col px-1 py-1 overflow-auto">
        {events.map(event => (
          <button key={event.title} className="flex items-center flex-shrink-0 h-5 px-1 text-xs hover:bg-gray-200">
            <span className={`flex-shrink-0 w-2 h-2 ${event.confirmed ? confirmedCss : unconfirmedCSss} rounded-full`}></span>
            <span className="ml-2 font-light leading-none">{event.time}</span>
            <span className="ml-2 font-medium leading-none truncate">{event.title}</span>
          </button>
        ))}
      </div>
